...
from cairn.core.models import BackgroundShell, cairnDeps, ToolResult
from cairn.tools.common import DEFAULT_BASH_TIMEOUT_MS, MAX_OUTPUT_CHARS


async def bash(
    ctx: RunContext[cairnDeps],
    command: str,
...
async def kill_shell(ctx: RunContext[cairnDeps], shell_id: str) -> ToolResult:
...
async def _bash_background(ctx: RunContext[cairnDeps], command: str, description: str) -> ToolResult:
