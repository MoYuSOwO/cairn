...
from .core.models import cairnDeps
...
def create_app(cwd: str | None = None) -> Any:
    try:
        ...
        raise ImportError("server 模式需要安装 fastapi: pip install cairn[server]")
    ...
    deps = cairnDeps(config=config, cwd=cwd, fs=fs)
    ...
    app = FastAPI(title="cairn Server")
...
def run_server(host: str = "127.0.0.1", port: int = 8720, cwd: str | None = None) -> None:
    try:
        ...
        raise ImportError("server 模式需要安装 uvicorn: pip install cairn[server]")
