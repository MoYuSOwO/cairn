...
└──────┬───────┘
       ...
       ▼
┌──────────────┐
│ cairn/tui   │  Textual TUI
└──────┬───────┘
       │  build_runtime()
       ▼
┌──────────────┐     ┌──────────────┐
│ cairn/core  │◀───▶│ cairn/tools │
└──────────────┘     └──────────────┘
```
...
- `cairnDeps`：依赖注入容器（增加 `event_bus`、`ask_user_service`、`subagent_service`）

### cairn/core/config.py
配置管理（~/.cairn/config.json、AGENTS.md、MCP 配置查找）。

### cairn/core/mcp.py
MCP toolsets 加载与缓存：
- `load_mcp_toolsets(cwd)`：启动时加载；按配置路径缓存；支持 `cairn_MCP_STRICT=1` 严格模式。
